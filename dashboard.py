...
def get_current_menu(week_number):
    ...
    for i in range(5):
        ...
        def dish_found():
            ...
            previous_menus = [
                menus[d.strftime("%d-%m-%Y")][st.session_state["skips"].get(d.strftime("%d-%m-%Y"), 0)] for d in
                previous_dates
            ]
...
with col2:
    ...
    num_students = cols2_2[0].number_input("Nombre d'élèves inscrits à la cantine :", min_value=0, max_value=1500,
                                           value=150)
    ...
    cols2_1_metrics[2].metric("Coût total semaine optimisé",
                              "{:2.1f}€".format(prix_semaine * num_students * total_etudiants / 5))
...
with col3:
    ...
    delay_main_dish = col3.slider("Délai d'apparition entre deux plats identiques (en jours)", min_value=1,
                                  max_value=30, value=7, step=1)
    delay_menu = col3.slider("Délai d'apparition entre deux menus identiques (en jours)", min_value=1, max_value=90,
                             value=30, step=1)